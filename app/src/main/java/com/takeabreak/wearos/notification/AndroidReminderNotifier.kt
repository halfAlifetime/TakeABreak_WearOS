...
class AndroidReminderNotifier(
...
) : ReminderNotifier, ReminderSelfTest {
    ...
    override fun showStatusNotification(state: TimerState) {
        ...
        val builder = NotificationCompat.Builder(context, NotificationChannels.CHANNEL_STATUS_ID)
            .addExtras(Bundle().apply { putString(NotificationActions.EXTRA_SESSION_ID, state.sessionId) })
        ...
        if (isRunning) {
            ...
            val pauseIntent = Intent(context, NotificationActionReceiver::class.java).apply {
                action = NotificationActions.PAUSE
                data = Uri.parse("takeabreak://notification/${state.sessionId}")
                putExtra(NotificationActions.EXTRA_SESSION_ID, state.sessionId)
            }
            ...
            val resumeIntent = Intent(context, NotificationActionReceiver::class.java).apply {
                action = NotificationActions.RESUME
                data = Uri.parse("takeabreak://notification/${state.sessionId}")
                putExtra(NotificationActions.EXTRA_SESSION_ID, state.sessionId)
            }
        ...
        val stopIntent = Intent(context, NotificationActionReceiver::class.java).apply {
            action = NotificationActions.STOP
            data = Uri.parse("takeabreak://notification/${state.sessionId}")
            putExtra(NotificationActions.EXTRA_SESSION_ID, state.sessionId)
        }
    ...
    override fun showPhaseReminder(
        ...
        val builder = NotificationCompat.Builder(context, channelId)
            .addExtras(Bundle().apply { putString(NotificationActions.EXTRA_SESSION_ID, sessionId) })
    ...
    override fun run(phase: TimerPhase): ReminderSelfTestResult =
        DefaultReminderSelfTest(object : ReminderSelfTestDevice {
