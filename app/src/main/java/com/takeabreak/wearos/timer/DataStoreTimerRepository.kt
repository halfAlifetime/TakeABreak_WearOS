...
class DataStoreTimerRepository internal constructor(private val dataStore: DataStore<Preferences>) : TimerRepository {
    constructor(context: Context) : this(context.timerDataStore)
    ...
    override val timerStateFlow: Flow<TimerState> = dataStore.data.map { prefs ->
    ...
    override suspend fun updateTimerState(transform: (TimerState) -> TimerState): TimerState {
        ...
        dataStore.edit { prefs ->
            ...
            if (updatedState.status == TimerStatus.ERROR) {
                prefs[PreferencesKeys.FAILURE_REASON] = (updatedState.failureReason ?: TimerFailure.UNKNOWN).storageCode
            } else {
                prefs.remove(PreferencesKeys.FAILURE_REASON)
            }
    ...
    private fun mapPreferencesToTimerState(prefs: Preferences): TimerState {
        ...
        val status = runCatching { TimerStatus.valueOf(statusStr) }.getOrDefault(TimerStatus.STOPPED)
        ...
        return TimerState(
            ...
            failureReason = TimerFailure.fromPersisted(status, prefs[PreferencesKeys.FAILURE_REASON])
        )
    }
}
