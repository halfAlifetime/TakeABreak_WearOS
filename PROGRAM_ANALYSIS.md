...
- `ActionFeedback` 明确携带来源与递增 ID；ViewModel 负责生成和匹配消费，导航层按来源分发。设置页显示工作/休息保存错误并滚动至提示；同字段成功保存清除旧错误，其他字段错误和期间产生的自检反馈保留。提示不再跨页面误显示。
- “去设置”先消费原反馈再启动系统页面；后续关闭动作只匹配原 ID，不会清除新的跳转失败提示。相同文案的两次反馈也具有不同 ID。
- `AndroidReminderNotifier` 的两处清理不再丢弃异常结果：普通异常记录操作及堆栈，定向清理附会话 ID；保留尽力清理，使旧提醒清理失败不阻止后续阶段振动和通知尝试，取消异常继续传播。
- 新增 7 项 ViewModel 回归，保留原 174 项，合计 181 项全部通过，0 失败、0 错误、0 跳过。调试 APK 构建成功，Lint 0 错误、34 警告且分类未增加。修复前 3 个用例分别复现工作/休息错误残留和旧关闭操作误删新反馈；证据为 `app/build/feedback-review/before-fix.log`、`final-validation.log`。

本轮仅完成本地验证；通知清理日志路径经过代码检查和构建，未注入真实 Android 通知服务故障。设置页新增提示的圆屏外观及交互尚未真机验证，未连接或安装手表。下文的真机触感确认仍对应此前 `b9597c4`。

**2026-09-23 原则复查后的历史修正（7320ac9）**

在 `211d054` 基线上复核了平台异常与首次加载问题。三个正式回归用例在修复前分别因通知清理异常、调度异常、加载占位状态失败；修复后通过。没有使用子代理。

- `TimerSystemEffects` 统一处理计时引擎使用的系统端口：调度异常转为已有 ALARM_SCHEDULING 分支，清理/通知异常记录日志并允许状态提交继续，CancellationException 不作为普通平台故障吞掉。停止先发布 STOPPED 并废弃旧会话；清理失败不等于系统对象一定已移除，但旧事件不能重新推进已停止会话。
- `TimerViewModel.timerState` 在读取完成前为 null，页面显示加载状态并不提供计时操作，命令入口也阻止加载期修改。引擎再次检查开始合法性，保留 PAUSED/ERROR 的会话与进度。此前“通知停止请求已记录、主状态仍不可读取后显式开始新会话”的恢复场景保持兼容，已有测试继续覆盖。
- 通知 action 解码由 `NotificationCommandHandler` 负责，引擎仅接收 `TimerCommand` 和不含设置跳转目标的继续检查结果。会话校验、继续条件检查与状态修改仍在同一把锁内；原通知常量、DataStore 和停止记录键未变。
- 原有 166 项测试保留；新增平台异常回归 7 项、加载竞态回归 1 项，合计 174 项全部通过。调试 APK 构建成功，Lint 0 错误、34 警告。验证日志：`app/build/review-principles/regression-before.log`、`final-validation.log`。

本次仅完成本地验证，未连接手表或覆盖安装。故障注入验证的是软件状态处理，不表示手表曾发生对应平台异常；新增加载界面的真机外观、修复版本的通知操作与触感仍未复测。下文的安装指纹和用户触感确认属于此前版本。

**2026-09-23 结构优化后的历史结论（P1–P5 本地完成）**

依据 AGENTS 的 Codex 可读性与功能模块化原则，仍保留一个 app 模块及原有计时事务边界。重构由单一代理执行，没有调用子代理；用户随后提供设备地址，已按计划开始新版真机验收。
...
真机进展：15:41:47 覆盖安装 b9597c4 对应 APK，原会话与截止时间保持。新版两次手动振动和两次真实自动提醒均有系统 finished 记录及用户触感确认。PRIORITY 勿扰下自检被阻止且没有新振动请求；暂停、继续、弹窗输入隔离、旧会话 STOP 与界面时长设置也已验证。

当前边界：最后的 ERROR 界面夹具已显示，但重试后的即时读取未满足断言，随后无线离线；尚未确认恢复结果与最终停止清理，冷进程通知停止 UI 也待完成。此前短周期已清理，系统设置和时长已恢复，不能据此宣称后续模拟故障已清除。渠道限制的设备映射、其他勿扰分支、整机重启和长期耗电未覆盖。API 30–32 分支保留且已编译，未在旧设备实测。具体证据见 DEVICE_DEBUG_REPORT。
